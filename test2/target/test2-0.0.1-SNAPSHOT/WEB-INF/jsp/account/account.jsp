...
<script type="text/javascript">
	...
	function modify() {
		...
		if (selected.length > 0) {
			location.href = getRootName() + '/account/editAccount.do?operate=modify&id=' + selected[0].id;
		} else {
			alert('请选择一条数据!');
		}
	}
...
<body>
	...
	<div class="manage_bg">
		查询条件：<input type="text" id="condition"/>
		<input type="button" value="查询" class="btn70" onclick="search();" />
		<input type="button" value="添加" class="btn70" onclick="location.href=getRootName() + '/account/editAccount.do?operate=add';" /> 
		<input type="button" value="编辑" class="btn70" onclick="modify();" />
		<input type="button" value="统计" class="btn70" onclick="location.href=getRootName() + '/account/statAccount.do;'" />
	</div>
