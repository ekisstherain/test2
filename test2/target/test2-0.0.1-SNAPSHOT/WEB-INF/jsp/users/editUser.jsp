...
<script type="text/javascript">
	...
	function addUsers() {
		...
		$.post(getRootName() + '/users/addUsers.do', param, function(res) {
			if (res.success) {
				alert('创建成功!');
	...
	function initModifyUser() {
		...
		if (id) {
			$.post(getRootName() + '/users/getUserById.do', {
				id : id
			}, function(res) {
				if (res) {
	...
	function modifyUsers() {
		...
		param.role = $('#roleList').val();
		$.post(getRootName() + '/users/modifyUser.do', param, function(res) {
			if (res) {
				alert('修改成功!');
				window.history.back(-1);
			} else {
	...
	function initRoleList() {
		return setSelectListData({
			...
			url : getRootName() + '/users/getRoleList.do',
			select : $('#roleList')
		});
	}
</script>
