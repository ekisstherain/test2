...
<script type="text/javascript">
	...
	function login() {
		var res = validate();
		if (res === true) {
			$.post(getRootName() + "/login.do", $("form").serializeObject(), function(result) {
				if (result && result.success) {
					window.location.href = getRootName() + "/index.do";
				} else {
	...
	$(function() {
		// 检测是否登录过了
		$('body').hide();
		$.post(getRootName() + '/getLoginInfo.do', function(result) {
			if (result.success) {
	...
	<div id="formwrapper">
		<form>
			<fieldset>
				<legend>用户登录</legend>
				<div>
					<label for="name">帐 号:</label> <input id="account" name="account" type="text" /> <br>
				</div>
				<div>
					<label for="password">密 码:</label> <input id="password" name="password" type="password" /> <br>
				</div>
				<div class="cookiechk">
					<label></label> <input id="login" type="button" class="buttom" value="登 录" /> <span id="error"></span>
				</div>
			</fieldset>
		</form>
	</div>
